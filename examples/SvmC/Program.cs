/*
 * This sample program is ported by C# from examples\svm_c_ex.cpp.
*/
    ...
    internal class Program
    {
        ...
        private static int Main()
        {
            ...
            using (var normalizer = new VectorNormalizer<SampleType>())
            {
                ...
                // type.
                using (var trainer = new SvmCTrainer<double, RadialBasisKernel<double, Matrix<double>>>())
                {
                    ...
                    using (var kernel = new RadialBasisKernel<double, Matrix<double>>(0.15625, 0, 0))
                    {
                        ...
                        using (var function = Dlib.TrainProbabilisticDecisionFunction<double,
                            ...
                            SvmCTrainer<double, RadialBasisKernel<double, Matrix<double>>>>(trainer, samples, labels, 3))
                        {
                        ...
                        using (var reduced = Dlib.Reduced2<double,
                                                           RadialBasisKernel<double, Matrix<double>>,
                                                           SvmCTrainer<double, RadialBasisKernel<double, Matrix<double>>>>(trainer, 10))
                        {
                            Console.WriteLine();
