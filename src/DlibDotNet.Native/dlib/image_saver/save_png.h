...
dlib::save_png(*((array2d<__TYPE__>*)image), std::string(file_name, file_name_length));
...
dlib::save_png(mat, std::string(file_name, file_name_length));\

#pragma endregion template

DLLEXPORT int save_png(array2d_type type, void* image, const char* file_name, const int file_name_length)
{
    int error = ERR_OK;
...
DLLEXPORT int save_png_matrix(matrix_element_type type,
                              void* matrix,
