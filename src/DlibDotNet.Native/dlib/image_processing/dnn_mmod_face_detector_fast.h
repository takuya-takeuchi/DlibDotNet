#ifndef _CPP_DNN_MMOD_FACE_DETECTOR_FAST_H_
#define _CPP_DNN_MMOD_FACE_DETECTOR_FAST_H_
...
using net_type_fast = loss_mmod<con<1, 9, 9, 1, 1, rcon5<rcon5<rcon5<downsampler<input_rgb_image_pyramid<pyramid_down<2>>>>>>>>;
...
DLLEXPORT net_type_fast* get_dnn_mmod_face_detector_fast(const char* model_filename)
{
	net_type_fast* net = new net_type_fast();
	dlib::deserialize(model_filename) >> (*net);
	return net;
}

DLLEXPORT int dnn_mmod_face_detector_fast_operator(
	net_type_fast* net,
...
DLLEXPORT void dnn_mmod_face_detector_fast_delete(net_type_fast* net)
{
	delete net;
}
