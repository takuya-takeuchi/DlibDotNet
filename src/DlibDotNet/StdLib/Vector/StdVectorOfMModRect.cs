...
namespace DlibDotNet
{
    ...
    public sealed class StdVectorOfMModRect : StdVector<MModRect>
    {

        #region Constructors

        public StdVectorOfMModRect()
        {
            this.NativePtr = Native.stdvector_mmod_rect_new1();
        }

        public StdVectorOfMModRect(int size)
        {
            if (size < 0)
        ...
        public StdVectorOfMModRect(IEnumerable<MModRect> data)
        {
            if (data == null)
        ...
        internal StdVectorOfMModRect(IntPtr ptr)
        {
            if (ptr == IntPtr.Zero)
